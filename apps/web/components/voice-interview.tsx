...
export function VoiceInterview({ onBack }: VoiceInterviewProps) {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        setErrorMessage('Failed to load voice interview configuration.');
      } finally {
        setIsLoadingConfig(false);
      }
    };
  ...
  const startVoiceInterview = async () => {
    if (!agentId) {
      setErrorMessage('Voice interview is currently unavailable. Please contact support.');
      return;
    }
            ...
            {!isConnected ? (
              <>
                ...
                <Button 
                  ...
                  disabled={!agentId || isLoadingConfig}
                  className="px-8 py-3 text-base"
                  size="lg"
                >
                  <PlayIcon className="w-5 h-5 mr-2" />
                  {isLoadingConfig ? 'Loading...' : 'Start Voice Interview'}
                </Button>
              </>
            ) : (
              <>
